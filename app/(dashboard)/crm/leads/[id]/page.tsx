    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-300">
            <Target className="h-6 w-6" />
          </span>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-800">{lead.title}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <ToastActionForm action={convertLeadToClient} successMessage="Lead converted.">
            <input type="hidden" name="leadId" value={lead.id} />
            <SubmitButton className="rounded-lg bg-gradient-to-r from-sky-500 to-blue-500 px-5 py-2.5 text-sm font-semibold text-white shadow-[0_4px_12px_rgba(59,130,246,0.3)] transition hover:shadow-[0_6px_16px_rgba(59,130,246,0.4)] active:scale-[0.98]">Convert</SubmitButton>
          </ToastActionForm>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Est. Value", value: <Money cents={lead.valueCents} />, icon: DollarSign, tone: "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-300" },
          { label: "Close Date", value: lead.expectedCloseDate ? <DateText value={lead.expectedCloseDate} /> : "—", icon: Calendar, tone: "bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-300" },
          { label: "Activities", value: activities.length, icon: Activity, tone: "bg-sky-50 dark:bg-sky-950/40 text-sky-600 dark:text-sky-300" },
          { label: "Contacts", value: contacts.length, icon: User, tone: "bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-300" },
        ].map((stat) => {
      ...
      <section className="grid gap-6 xl:grid-cols-[1.4fr_0.6fr]">
        <div className="rounded-xl border border-sky-100 bg-white/95 p-5 shadow-[0_14px_40px_rgba(31,92,132,0.10)]">
          <div className="grid gap-5 md:grid-cols-4">
            {[
              ...
              return (
                <div key={item.label} className="rounded-xl bg-sky-50 p-4">
        ...
        <div className="rounded-xl border border-sky-100 bg-white/95 p-5 shadow-[0_14px_40px_rgba(31,92,132,0.10)]">
          ...
          <ToastActionForm action={updateLeadStage} successMessage="Stage updated." className="mt-4 grid gap-3">
            <input type="hidden" name="leadId" value={lead.id} />
            <select name="stageId" defaultValue={lead.stageId ?? ""} className="h-11 rounded-lg border border-sky-100 px-3 text-sm outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-500/20">
              {stages.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            <SubmitButton className="rounded-lg bg-gradient-to-r from-sky-500 to-blue-500 px-5 py-2.5 text-sm font-semibold text-white shadow-[0_4px_12px_rgba(59,130,246,0.3)] transition hover:shadow-[0_6px_16px_rgba(59,130,246,0.4)] active:scale-[0.98]">Update</SubmitButton>
          </ToastActionForm>
          {workOrderRows[0] && (
            <Link href={`/work-orders/${workOrderRows[0].id}`} className="mt-4 inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-sky-500 to-blue-500 px-4 py-2.5 text-sm font-semibold text-white shadow-[0_4px_12px_rgba(59,130,246,0.3)] transition hover:shadow-[0_6px_16px_rgba(59,130,246,0.4)] active:scale-[0.98]">
        ...
        <div className="rounded-xl border border-sky-100 bg-white/95 p-5 shadow-[0_14px_40px_rgba(31,92,132,0.10)]">
          ...
          <div className="mt-4 grid gap-3">
            {contacts.map((contact) => (
              <div key={contact.id} className="rounded-xl border border-sky-100 p-4">
                <p className="font-bold text-slate-800">{contact.name}</p>
                <p className="mt-1 text-sm text-slate-400">{contact.title || "—"}</p>
                {contact.email && <p className="mt-3 flex items-center gap-1.5 text-sm text-slate-400"><MessageSquare className="h-3.5 w-3.5" />{contact.email}</p>}
                <p className="flex items-center gap-1.5 text-sm text-slate-400">{contact.whatsappNumber || contact.phone || "—"}</p>
              </div>
            ))}
            ...
            {activities.map((activity) => (
              <div key={activity.id} className="flex gap-3">
                ...
                <div className="min-w-0 flex-1 border-b border-sky-100 pb-4">
                  ...
                  <p className="mt-0.5 text-xs text-slate-400">{activity.userName || "System"}</p>
                </div>
              </div>
            ))}
            {!activities.length && <p className="py-6 text-center text-sm text-slate-400">No activity</p>}
          </div>
        </div>
      </section>
      ...
      <div className="rounded-xl border border-sky-100 bg-white/95 shadow-[0_14px_40px_rgba(31,92,132,0.10)]">
        ...
        <div className="p-5">
          <DataTable headers={["Invoice", "Due", "Status"]} empty="No invoices." rows={invoices.map((inv) => [
            <span key="n" className="font-mono text-sm font-bold text-slate-800">{inv.invoiceNumber}</span>,
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-xl border border-sky-100 bg-white/95 p-5 shadow-[0_14px_40px_rgba(31,92,132,0.10)]">
          <h2 className="font-bold text-slate-800">Stage History</h2>
          <div className="mt-4 grid gap-3">
            {stageHistory.map((item) => (
              <div key={item.id} className="rounded-xl border border-sky-100 p-4 text-sm">
                <p className="font-bold text-slate-800">Stage updated</p>
                <p className="mt-1 text-slate-400">{formatDateTime(item.createdAt)} by {item.userName || "System"}</p>
              </div>
            ))}
            {!stageHistory.length && <p className="py-6 text-center text-sm text-slate-400">No changes</p>}
          </div>
        </div>
